...
fail() {
  printf 'runtime artifact check failed: %s\n' "$1" >&2
  exit 1
}
...
printf '[runtime:artifact:check] NYX_RUNTIME_PATH=%s\n' "$ARTIFACT_PATH"

NYX_RUNTIME_PATH="$ARTIFACT_PATH" \
  NYX_RUNTIME_ARTIFACT_HEALTH=1 \
  pnpm --dir "$ROOT/apps/desktop" exec vitest run electron/main/runtime/health.integration.test.ts
